...
def calculate_cost():
    ...
    # Correctly handle the column name for square footage in the database
    if square_footage >= 5000:
        sf_column_name = '"5000+"'
    else:
        # Round down to the nearest hundred for square footage
        rounded_sf = square_footage - (square_footage % 100)
        if rounded_sf < 700:
            rounded_sf = 700  # Ensure it doesn't go below the minimum column
        sf_column_name = f'"{rounded_sf}"'
    ...
    if cost:
        # Multiply by the square footage to get the total cost
        total_cost = float(cost[0]) * square_footage
